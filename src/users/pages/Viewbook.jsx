...
function Viewbook() {
  ...
  const makePayment = async()=>{
     console.log(viewBookDetails);
     //object instance
     const stripe = await loadStripe('pk_test_51RTHpoRZmPjsyUoYSAjti1xdAOx4EJyeKZueDnHp5zctUWtqSfZHaye3Pjp9ba1AeLO8i3GMHfCtjX9iiy4BEKGk00j5ckIp1W');
     //date to update in backend
     const reqBody = {
          ...
          <div className="md:grid grid-cols-[1fr_3fr] w-full overflow-x-hidden"> <div>
            ...
            <div className='px-4 mt-5 md:mt-0'>
              ...
              <div className='mt-10 flex justify-end' >
               <Link to={'/all-books'}>
                  <button className='px-4 py-3 bg-blue-800 rounded text-white hover:bg-white hover:text-blue-800
                  hover:border hover: border-blue-800'><FontAwesomeIcon className='me-2' icon={faBackward} /> Back
                  </button>
               </Link>
                <button type='button' onClick={makePayment} className=' ms-3 px-4 py-3 bg-green-800 rounded text-white hover:bg-white
              hover:text-green-800 hover:border hover:border-green-800'>Buy $ {viewBookDetails?.dprice}</button>
              </div>
            </div>
          </div>
        </div>
      </div>
      {modalStatus && <div onClick={() => setModalStatus(false)} className="relative z-10"
        aria-labelledby="modal-title" role="dialog" aria-modal="true">
        <div className="fixed inset-0 bg-gray-500/75 transition-opacity" aria-hidden="true"></div>
        <div className="fixed inset-0 Z-10 w-screen overflow-y-auto">
          <div className="flex md:min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
            <div className="relative transform overflow-hidden rounded-lg bg-white text-left shadow-x1 transition-all sm: my-8 sm:w-full sm:max-w-4x1">
              ...
              <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <p className='text-blue-500'><FontAwesomeIcon icon={faCamera} className='me-3' />Camera click of the
                  book in the hand of seller</p>
                <div className='md: flex my-4'>
                 {
                  viewBookDetails?.uploadedImg.map((item)=>(
                     <img className='mt-4 mx-5' src={`${serverUrl}/upload/${item}`} alt="no 
